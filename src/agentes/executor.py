...
class SistemaAgentes:
    ...
    def _pipeline_nivel1(
        ...
        if resultado_ferramenta:
            ...
            if resultado_ferramenta.startswith("Confirmação necessária"):
                ...
                self._salvar(
                    pergunta, resultado_ferramenta, nome_agente, nivel=1,
                    inicio=inicio, cachear=False,
                )
                self._registrar_execucao(nome_agente, 1, "ferramenta")
            return resultado_ferramenta, []

        # 1a (fallback). Ferramentas heuristicas (baratas; mesmo motivo: nao cachear).
        ...
        if resultado_heuristico:
            ...
            self._salvar(
                pergunta, resultado_heuristico, nome_agente, nivel=1,
                inicio=inicio, cachear=False,
            )
            self._registrar_execucao(nome_agente, 1, "ferramenta")
    ...
    def _executar_ferramenta_por_intencao(
        ...
        if (
            intencao.ferramenta == "arquivo"
            and params.get("acao") == "criar"
            and not self._acao_local_confirmada
            and not AUTO_APROVAR_ACOES_LOCAIS
        ):
            # Comandos sao filtrados pela allowlist (comando_permitido) no proprio
            # bloco de "comando"; aqui so a criacao de arquivo via intencao exige
            # confirmacao quando a heuristica de texto nao detectou a acao mutavel.
            return (
